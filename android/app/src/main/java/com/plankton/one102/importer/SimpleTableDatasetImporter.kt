...
private const val SIMPLE_TABLE_FIRST_POINT_COL = 2 // C (0-based)
...
private fun findTotalRowIndex(sheet: Sheet, startRow: Int, maxScanRows: Int, formatter: DataFormatter, evaluator: FormulaEvaluator?): Int? {
    ...
    for (r in startRow..last) {
        val a = cellText(sheet.getRow(r)?.getCell(0), formatter, evaluator)
        val b = cellText(sheet.getRow(r)?.getCell(1), formatter, evaluator)
        if (a.contains("总计") || b.contains("总计") || a.contains("合计") || b.contains("合计")) return r
    }
    return null
}
 ...
 * - 第3行：名称（点位列）从 C 列开始
 * - 第4行：浓缩体积（ml）从 C 列开始
 * - 物种行：A列=大类（可留空表示同类），B列=物种名，C列开始为计数
 */
suspend fun importDatasetFromSimpleTableExcel(
    ...
    contentResolver.openInputStream(uri)?.use { input ->
        ...
        try {
            val sheet = wb.sheetIterator().asSequence().firstOrNull { it.sheetName.contains("计数") }
                ?: wb.getSheetAt(0)
                ?: error("无法读取工作表")
            ...
            var c = SIMPLE_TABLE_FIRST_POINT_COL
            while (c <= SIMPLE_TABLE_FIRST_POINT_COL + 200) {
            ...
            if (pointLabels.isEmpty()) throw IllegalArgumentException("未识别到任何点位列（请确认是简略计数表格式）")

            var dataStartRow = concRow + 1
            val origRowText = cellText(sheet.getRow(dataStartRow)?.getCell(0), formatter, evaluator)
            val origValues = if (origRowText.contains("原水")) {
                val values = mutableListOf<Double?>()
                for (i in pointLabels.indices) {
                    values += cellDouble(sheet.getRow(dataStartRow)?.getCell(SIMPLE_TABLE_FIRST_POINT_COL + i), formatter, evaluator)
                        ?.takeIf { it > 0 }
                }
                dataStartRow += 1
                values
            ...
            val points: List<Point> = pointLabels.mapIndexed { idx, label ->
                ...
                Point(
                    ...
                    vOrigL = origValues?.getOrNull(idx)?.takeIf { it.isFinite() && it > 0 } ?: defaultVOrigL,
                    site = site,
                    depthM = depthM,
                )
            }
            ...
            val totalRow = findTotalRowIndex(sheet, dataStartRow, maxScanRows = 5000, formatter, evaluator)
                ?: (sheet.lastRowNum + 1)

            var lvl1 = ""
            val species = mutableListOf<Species>()
            for (r in dataStartRow until totalRow) {
                val row = sheet.getRow(r) ?: continue
                val lvl1Cell = cellText(row.getCell(0), formatter, evaluator)
                val nameCn = cellText(row.getCell(1), formatter, evaluator)

                if (lvl1Cell.isNotBlank()) {
                    lvl1 = normalizeLvl1Name(lvl1Cell)
                }
                ...
                val counts = buildMap {
                    for (i in points.indices) {
                        val cell = row.getCell(SIMPLE_TABLE_FIRST_POINT_COL + i)
