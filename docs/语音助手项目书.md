...
本轮 3.8 发布：保留 3.7 的可恢复任务队列与 v2 Binder 合作方接口；设置页将“软件更新（GitHub）”作为独立入口，检查后明确显示当前已是最新版本或可下载版本。主 App 仍是唯一可以预览并确认业务写入的一方。
...
- 不自动写入任何合作方业务数据；语音助手只返回文本、质量和动作建议

v3.7 的现场录入提供“快速发送”和“核对后发送”两条明确路径：前者在转写完成后自动把结果交给接收 App，但仍保留接收 App 的预览确认；后者停留在可编辑文本、任务时间线和发送按钮。主界面与接入页均提供按住录音、松开结束、暂停/继续、取消和录音质量提示。

## 1.1 v3.7 架构与数据保护

- v2 合作方通过 Binder 会话调用，服务端每次依据 UID、包名和证书摘要校验调用方；首次调用由用户选择一次、限时或长期授权。
- `generic` Profile 只有通用转写；`plankton-v1` 仅在获 `domain_profile` 范围后接收受限词表和点位，仍只输出动作建议。
- 后台任务使用 Room 持久化。进程被中断后运行中任务回到队列；取消按任务 ID 生效；完成后音频引用最多保留 24 小时。
- 系统备份关闭；诊断不包含音频、全文转写、API Key 或调用方签名。更新和安装覆盖必须保持正式签名证书一致。
- 公开规范、错误码与迁移说明见 [v2 接口文档](api/voice-assistant-v2.md)。
