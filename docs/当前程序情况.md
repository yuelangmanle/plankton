...
- 当前版本：主 App `7.6`（`versionCode=760`）；语音助手 `3.9`（`versionCode=39`，独立正式发布）
...
### 2.0 主 App 7.6 截断修复记录（2026-07-27）

- AI 全文阅读不再使用预览长度，分类、湿重、主助手与全局助手的完整回答均可滚动查看；主助手首页回答不再强制限制为 10 行。
- 分类/湿重的初始输出预算为 2400 token；统一 API 调用会识别 OpenAI 兼容的 finish_reason=length，或仅收到思考过程时自动续写并要求最终结论。
- 同一服务重复给出 reasoning_content 与正文时不再重复包装为两段；思考过程仍保留折叠查看。
- Apilot 导入/发送直接启动 V2 Intent，移除会误判已安装应用的 resolveActivity 预检；只有实际启动失败才显示安装引导。

### 2.0.1 主 App 7.5 发布记录（2026-07-27）
...
### 2.0.2 主 App 7.4 / 语音助手 3.9 发布记录（2026-07-27）
